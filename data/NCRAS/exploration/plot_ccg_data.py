...
timeseries.suptitle("Monthly fractional breast cancer cases in London CCGs")
count = 0
for ax in axs.flat:
    ccg = ccgs[count]
    plot_df = cancer_df.loc[cancer_df.ccg_name==ccg]
    ax.plot(plot_df.date, plot_df.all_ages, alpha=0.7, c=f"C{count}")
    ax.set(xlabel="Time", ylabel="Cases as fraction of population (monthly)")
    ax.label_outer()
    ax.set_title(ccg)
    count += 1
...
histogram.suptitle("Monthly fractional breast cancer cases in London CCGs")
count = 0
for ax in axs.flat:
    ccg = ccgs[count]
    plot_df = cancer_df.loc[cancer_df.ccg_name == ccg]
    ax.hist(plot_df.all_ages, alpha=0.7, color=f"C{count}")
    ax.set(xlabel="Cases as fraction of population (monthly)")
    ax.label_outer()
    ax.set_title(ccg)
    count += 1
...
plot_filename = f"monthly_fractional_cases_ccg_timeseries.png"
timeseries.savefig(os.path.join(os.path.dirname(os.path.realpath(__file__)), plot_filename))
print(f"\nSaved {plot_filename}")

plot_filename = f"monthly_fractional_cases_ccg_histograms.png"
