...
no2_filenames = [file for file in listdir(no2_folder) if re.findall(f"averaged_ccgs.csv", file)]
print(no2_filenames)
...
for no2_file in no2_filenames:
    no2_df = pd.read_csv(join(no2_folder, no2_file)).set_index("MeasurementDateGMT").loc[
...
######################################################################## haven't finished editing this script
